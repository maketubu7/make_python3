...
class Action(object):
    ...
    letter_codes = [ord(ch) for ch in 'WASDRQCwasdrqc']
...
class Screen(object):
    ...
    help_string2 = '(R)Restart (Q)Exit  (C)Contine after win'
...
class GameManager(object):
    ...
    def _restart_or_exit_or_continue(self):
        self.screen.draw()
        action = 'over'

        if self.action.get() == Action.RESTART:
            action = 'init'
        if self.action.get() == Action.CONTINUE:
            action = 'continue'
        return action
            

    def state_win(self):
        return self._restart_or_exit_or_continue()
