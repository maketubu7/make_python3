...
def dealCallGraphUnDirected():
    ...
    #F:\make_python3\data\call_detail.csv
    call = pd.read_csv("F:\make_python3\data\call_detail.csv")
    call.columns = callSchema
    call[["start_phone", "end_phone"]] = call[["start_phone", "end_phone"]].astype(str)
    call[["num"]] = call[["num"]].astype(int)

    G = nx.from_pandas_edgelist(call,"start_phone",'end_phone',['num'])
    ...
    print("最短路径: ",nx.shortest_path(G,source="15860771674",target="18570546027"))
    print("最短距离: ",nx.dijkstra_path_length(G,source="15860771674",target="18570546027"))

    # 所有节点的最短路径
    all_short = nx.all_pairs_shortest_path(G)
    for k,v in dict(all_short).items():
        print(k,v)
        break

    #网络结构的度 拿到最大的度的节点
    degree = G.degree()
    degree = sorted(degree,key=lambda tp: tp[1],reverse=True)
    print(degree)
    ...
    # print("图的密度",nx.diameter(G))

    # nx.draw(G)
    print(nx.is_connected(G))

    #最大连通子图最大连通子图
    # max_connectsub = nx.sub(G)
...
def dealCallGraphDirected():
    ...
    G = nx.from_pandas_edgelist(call, "start_phone", "end_phone", ["num"], create_using=nx.DiGraph())
    ...
    # print("包含某些点的子图：",subgraph)


    # 寻找初度为0的边
    out_zero = [v for v in G.out_degree() if v[1] == 0]
    print('出度为0的边：', out_zero)

    out_two = [v for v in G.out_degree() if v[1] > 2]
    print('出度大于2的边：', out_two)

    in_two = [v for v in G.in_degree() if v[1] > 2]
    print('入度大于2的边：', in_two)

    degree_three = [v for v in G.degree() if v[1] > 3]
