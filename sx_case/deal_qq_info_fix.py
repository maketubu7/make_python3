...
save_path = 'C:\\Users\\lenovo\\Desktop\\case_data_all\\{}'
...
def get_allinfo(path):
    res = []
    for i in range(4):
        with open(add_save_path(path+"_%s"%i),encoding='utf-8-sig',errors='ignore') as f:
            print(path+"_%s"%i)
            res.append(json.load(f,strict=False))
    df = pd.DataFrame(functools.reduce(lambda a,b:a+b,res))
    df.to_csv(save_path+'%s.csv'%path,header=True,index=None)
...
if __name__ == '__main__':
    ...
    get_allinfo('wechat_groupmsg_banknum')